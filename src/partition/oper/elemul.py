...
"""Partition elementwise multiply operators into behavior-level tasks."""
...
def elemul_data(
    ...
    """Create data for element-wise multiplication operation.
...
def Elemul(
    ...
    """Initialize and build an element-wise multiplication operator.
    ...
    Returns:
        ...
        elemul_instance: The initialized elemul operator instance
    """
    source_data_tag_list, target_data_tag_list, next_online_data_tag, next_offline_data_tag = elemul_data(
        data_dict=data_dict,
        oper_name=oper_name,
        oper_split_list=oper_split_list,
    ...
    elemul_instance.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    next_oper_tag = oper_tag[:-1] + (oper_tag[-1]+1,)
    next_beha_offset = beha_tag_offset + elemul_instance.degrees

    return next_oper_tag, next_online_data_tag, next_offline_data_tag, next_beha_offset, elemul_instance
