...
"""Partition vector add operators into behavior-level tasks."""
...
def vecadd_data(
    ...
    """Create data for vector addition operation.
...
def Vecadd(
    ...
    """Initialize and build a vector addition operator.
    ...
    Returns:
        ...
        vecadd_instance: The initialized vecadd operator instance
    """
    source_data_tag_list, target_data_tag_list, next_online_data_tag, next_offline_data_tag = vecadd_data(
        data_dict=data_dict,
        oper_name=oper_name,
        oper_split_list=oper_split_list,
    ...
    vecadd_instance.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    next_oper_tag = oper_tag[:-1] + (oper_tag[-1]+1,)
    next_beha_offset = beha_tag_offset + vecadd_instance.degrees

    return next_oper_tag, next_online_data_tag, next_offline_data_tag, next_beha_offset, vecadd_instance
...
if __name__ == "__main__":
    ...
    vectadd_next_oper_tag, next_online_data_tag, next_offline_data_tag, vectadd_next_beha_offset, vectadd_oper_instance = Vecadd(
        data_dict=data_dict,
        beha_dict=beha_dict,
        oper_name="",
