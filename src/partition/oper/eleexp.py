...
"""Partition elementwise exponent operators into behavior-level tasks."""
...
def eleexp_data(
    ...
    """Create data for element-wise exponential operation.
...
def Eleexp(
    ...
    """Initialize and build an element-wise exponential operator.
    ...
    Returns:
        ...
        eleexp_instance: The initialized eleexp operator instance
    """
    source_data_tag_list, target_data_tag_list, next_online_data_tag, next_offline_data_tag = eleexp_data(
        data_dict=data_dict,
        oper_name=oper_name,
        oper_split_list=oper_split_list,
    ...
    eleexp_instance.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    next_oper_tag = oper_tag[:-1] + (oper_tag[-1]+1,)
    next_beha_offset = beha_tag_offset + eleexp_instance.degrees

    return next_oper_tag, next_online_data_tag, next_offline_data_tag, next_beha_offset, eleexp_instance
