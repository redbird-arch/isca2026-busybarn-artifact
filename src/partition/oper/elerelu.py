...
"""Partition ReLU-style elementwise activation operators."""
...
class elerelu(elementwise_oper):
...
def elerelu_data(
    ...
    """Create data for element-wise ReLU operation.
...
def Elerelu(
    ...
    """Initialize and build an element-wise ReLU operator.
    ...
    Returns:
        ...
        elerelu_instance: The initialized elerelu operator instance
    """
    source_data_tag_list, target_data_tag_list, next_online_data_tag, next_offline_data_tag = elerelu_data(
        data_dict=data_dict,
        oper_name=oper_name,
        oper_split_list=oper_split_list,
    ...
    elerelu_instance.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    next_oper_tag = oper_tag[:-1] + (oper_tag[-1]+1,)
    next_beha_offset = beha_tag_offset + elerelu_instance.degrees

    return next_oper_tag, next_online_data_tag, next_offline_data_tag, next_beha_offset, elerelu_instance
