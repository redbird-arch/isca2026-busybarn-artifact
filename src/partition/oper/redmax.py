...
"""Partition reduction-max operators into behavior-level tasks."""
...
def redmax_data(
    ...
    """Create data for reduction max operation.
...
def Redmax(
    ...
    """Initialize and build a reduction max operator.
    ...
    Returns:
        ...
        redmax_instance: The initialized redmax operator instance
    """
    source_data_tag_list, target_data_tag_list, next_online_data_tag, next_offline_data_tag = redmax_data(
        data_dict=data_dict,
        oper_name=oper_name,
        oper_split_list=oper_split_list,
    ...
    redmax_instance.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    next_oper_tag = oper_tag[:-1] + (oper_tag[-1]+1,)
    next_beha_offset = beha_tag_offset + redmax_instance.degrees

    return next_oper_tag, next_online_data_tag, next_offline_data_tag, next_beha_offset, redmax_instance
...
if __name__ == "__main__":
    ...
    redmax_next_oper_tag, next_online_data_tag, next_offline_data_tag, redmax_next_beha_offset, redmax_oper_instance = Redmax(
        data_dict=data_dict,
        beha_dict=beha_dict,
        oper_name="",
