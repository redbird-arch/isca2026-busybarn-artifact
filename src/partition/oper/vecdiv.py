...
"""Partition vector divide operators into behavior-level tasks."""
...
def vecdiv_data(
    ...
    """Create data for vector division operation.
...
def Vecdiv(
    ...
    """Initialize and build a vector division operator.
    ...
    Returns:
        ...
        vecdiv_instance: The initialized vecdiv operator instance
    """
    source_data_tag_list, target_data_tag_list, next_online_data_tag, next_offline_data_tag = vecdiv_data(
        data_dict=data_dict,
        oper_name=oper_name,
        oper_split_list=oper_split_list,
    ...
    vecdiv_instance.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    next_oper_tag = oper_tag[:-1] + (oper_tag[-1]+1,)
    next_beha_offset = beha_tag_offset + vecdiv_instance.degrees

    return next_oper_tag, next_online_data_tag, next_offline_data_tag, next_beha_offset, vecdiv_instance
...
if __name__ == "__main__":
    ...
    test_vecdiv = vecdiv(
        ...
        target_data_tags=[(0, 2)],
        oper_split_list=[(1,), (128, 256, 640), (384, 384)], 
        vec_dims=[0, 2]
    )
    test_vecdiv.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )
