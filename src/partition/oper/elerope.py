...
"""Partition rotary positional embedding operators into behavior-level tasks."""
...
class elerope(elementwise_oper):
...
def Elerope(
    ...
    Returns:
        ...
        elerope_instance: The initialized elerope operator instance
    """
    source_data_tag_list, target_data_tag_list, next_online_data_tag, next_offline_data_tag = elerope_data(
        data_dict=data_dict,
        oper_name=oper_name,
        oper_split_list=oper_split_list,
    ...
    elerope_instance.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    next_oper_tag = oper_tag[:-1] + (oper_tag[-1]+1,)
    next_beha_offset = beha_tag_offset + elerope_instance.degrees

    return next_oper_tag, next_online_data_tag, next_offline_data_tag, next_beha_offset, elerope_instance
