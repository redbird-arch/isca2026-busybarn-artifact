...
"""Partition elementwise add operators into behavior-level tasks."""
...
def Eleadd(
    ...
    """Initialize and build an element-wise addition operator.
    ...
    Returns:
        ...
        eleadd_instance: The initialized eleadd operator instance
    """
    source_data_tag_list, target_data_tag_list, next_online_data_tag, next_offline_data_tag = eleadd_data(
        data_dict=data_dict,
        oper_name=oper_name,
        oper_split_list=oper_split_list,
    ...
    eleadd_instance.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    next_oper_tag = oper_tag[:-1] + (oper_tag[-1]+1,)
    next_beha_offset = beha_tag_offset + eleadd_instance.degrees

    return next_oper_tag, next_online_data_tag, next_offline_data_tag, next_beha_offset, eleadd_instance
...
if __name__ == "__main__":
    ...
    eleadd_next_oper_tag, next_online_data_tag, next_offline_data_tag, eleadd_next_beha_offset, eleadd_oper = Eleadd(
        data_dict=data_dict,
        beha_dict=beha_dict,
        oper_name="eleadd",
