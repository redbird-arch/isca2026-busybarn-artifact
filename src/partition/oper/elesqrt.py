...
"""Partition elementwise square-root operators into behavior-level tasks."""
...
class elesqrt(elementwise_oper):
...
def elesqrt_data(
    ...
    """Create data for element-wise square root operation.
...
def Elesqrt(
    ...
    """Initialize and build an element-wise square root operator.
    ...
    Returns:
        ...
        elesqrt_instance: The initialized elesqrt operator instance
    """
    source_data_tag_list, target_data_tag_list, next_online_data_tag, next_offline_data_tag = elesqrt_data(
        data_dict=data_dict,
        oper_name=oper_name,
        oper_split_list=oper_split_list,
    ...
    elesqrt_instance.build_dependency(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    next_oper_tag = oper_tag[:-1] + (oper_tag[-1]+1,)
    next_beha_offset = beha_tag_offset + elesqrt_instance.degrees

    return next_oper_tag, next_online_data_tag, next_offline_data_tag, next_beha_offset, elesqrt_instance
