...
"""Partition softmax operators into reduction and elementwise behaviors."""
...
class softmax(oper_notation):
    ...
    def __init__(self,
        ...
        '''
        softmax_oper uses online-softmax algorithm
        source_data_tags: [source_data_0]
        the parallel degree is decided by all source_data_0 dimensions 
        source_data_0: [dim_0, ..., dim_m, dim_k] -> [bb, br, bc]
    ...
    def update_target_data(self, 
        ...
        redmax_next_oper_tag, next_online_data_tag, next_offline_data_tag, redmax_next_beha_offset, redmax_oper = Redmax(
            data_dict=data_dict,
            beha_dict=beha_dict,
        ...
        redmax_ofmap_data_tag = redmax_oper.target_data_tags[0]

        vecadd_next_oper_tag, next_online_data_tag, next_offline_data_tag, vecadd_next_beha_offset, vecadd_oper = Vecadd(
            data_dict=data_dict,
            beha_dict=beha_dict,
            oper_name=self.oper_name,
            source_data_tags=[self.source_data_tags[0], redmax_ofmap_data_tag],
            oper_split_list=self.oper_split_list,
            oper_tag=self.oper_tag,
            online_data_tag=next_online_data_tag,
            offline_data_tag=next_offline_data_tag,
            vec_dims=self.reduce_dims,
            beha_tag_offset=redmax_next_beha_offset
        )
        vecadd_ofmap_data_tag = vecadd_oper.target_data_tags[0]

        eleexp_next_oper_tag, next_online_data_tag, next_offline_data_tag, eleexp_next_beha_offset, eleexp_oper = Eleexp(
            data_dict=data_dict,
            beha_dict=beha_dict,
            oper_name=self.oper_name,
            source_data_tags=[vecadd_ofmap_data_tag],
            oper_split_list=self.oper_split_list,
            oper_tag=self.oper_tag,
            online_data_tag=next_online_data_tag,
            offline_data_tag=next_offline_data_tag,
        ...
        eleexp_ofmap_data_tag = eleexp_oper.target_data_tags[0]

        redsum_next_oper_tag, next_online_data_tag, next_offline_data_tag, redsum_next_beha_offset, redsum_oper = Redsum(
            data_dict=data_dict,
            beha_dict=beha_dict,
            oper_name=self.oper_name,
            source_data_tags=[eleexp_ofmap_data_tag],
            oper_split_list=self.oper_split_list,
            oper_tag=self.oper_tag,
            online_data_tag=next_online_data_tag,
            offline_data_tag=next_offline_data_tag,
            reduce_dims=self.reduce_dims,
            beha_tag_offset=eleexp_next_beha_offset
        )
        redsum_ofmap_data_tag = redsum_oper.target_data_tags[0]

        vecdiv_next_oper_tag, next_online_data_tag, next_offline_data_tag, vecdiv_next_beha_offset, vecdiv_oper = Vecdiv(
            data_dict=data_dict,
            beha_dict=beha_dict,
            oper_name=self.oper_name,
            source_data_tags=[eleexp_ofmap_data_tag, redsum_ofmap_data_tag],
            oper_split_list=self.oper_split_list,
            oper_tag=self.oper_tag,
            online_data_tag=next_online_data_tag,
            offline_data_tag=next_offline_data_tag,
        ...
        vecdiv_data_tag = vecdiv_oper.target_data_tags[0]
        self.target_data_tags = [vecdiv_data_tag]

        return vecdiv_next_oper_tag, next_online_data_tag, next_offline_data_tag, vecdiv_next_beha_offset
...
def Softmax(
    ...
    softmax.online_data_tag = online_data_tag
    softmax.offline_data_tag = offline_data_tag

    softmax_next_oper_tag, next_online_data_tag, next_offline_data_tag, softmax_next_beha_offset = softmax_oper.update_target_data(
        data_dict=data_dict,
        beha_dict=beha_dict
    )

    return softmax_next_oper_tag, next_online_data_tag, next_offline_data_tag, softmax_next_beha_offset, softmax_oper
...
if __name__ == "__main__":
    ...
    softmax_next_oper_tag, next_online_data_tag, next_offline_data_tag, softmax_next_beha_offset, softmax_oper = Softmax(
        data_dict=data_dict,
        beha_dict=beha_dict,
        oper_name="",
